...
| 10 | ~~Test endpoints default to localhost — accepted for the current CI topology~~ | Hurl targets `http://localhost:8080` and Cypress defaults to `http://localhost:4173`. Tests run from the same CI runner as the published application ports, so localhost is an intentional default rather than a blocker. | **Medium** |
